...
class Model_suboutput extends CI_Model
{
	...
	{
		$query = "SELECT * FROM suboutput INNER JOIN divisi ON suboutput.nama_divisi = divisi.nama ORDER BY divisi.deskripsi ASC";
		return $this->db->query($query);
	}

	function tampilkan_data($iddivisi)
	{
		$query = "SELECT * FROM suboutput INNER JOIN divisi ON suboutput.nama_divisi = divisi.nama WHERE divisi.id_divisi=$iddivisi ORDER BY divisi.nama ASC";
		return $this->db->query($query);
	}
